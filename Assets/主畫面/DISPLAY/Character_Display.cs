...
public class Charactor : MonoBehaviour
{
    ...
    private List<Location> objectsDataList = new List<Location>();
    ...
    private void Update_Display(int Faction,int[] Lineup)
    {
        ...
        for(int i = 0; i<Lineup.Length-1; i++)
        {
            Pictures[7*(Faction-1)+Lineup[i]-1].rectTransform.anchoredPosition = new Vector3(objectsDataList[7*(Faction-1)+Lineup[i]-1].position.x+i*objectsDataList[7*(Faction-1)+Lineup[i]-1].offset,objectsDataList[7*(Faction-1)+Lineup[i]-1].position.y,objectsDataList[7*(Faction-1)+Lineup[i]-1].position.z);
        }
    }
